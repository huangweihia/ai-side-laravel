<?php $__env->startSection('title', isset($submission) ? '重新编辑投稿 - AI 副业情报局' : 'VIP 投稿 - AI 副业情报局'); ?>
...
<div class="container" style="max-width: 900px; margin: 0 auto; padding: 40px 20px;">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isEdit && $submission->review_note): ?>
    ...
    <form method="POST" action="<?php echo e($isEdit ? route('submissions.update', $submission->id) : route('submissions.store')); ?>" id="submission-form">
        <?php echo csrf_field(); ?>
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isEdit): ?>
            ...
            <div style="display: grid; gap: 20px;">
                <div>
                    ...
                    <input name="title" required maxlength="255" 
                           value="<?php echo e($isEdit ? old('title', $submission->title) : old('title')); ?>"
                ...
                <div>
                    ...
                    <textarea name="summary" rows="3" maxlength="500" 
                              ...
                              onfocus="this.style.borderColor='var(--primary)'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.15)'"
                              onblur="this.style.borderColor='rgba(255,255,255,0.15)'; this.style.boxShadow='none'"><?php echo e($isEdit ? old('summary', $submission->summary) : old('summary')); ?></textarea>
                    <div style="font-size: 12px; color: var(--gray); margin-top: 6px; text-align: right;">
                        <span id="summary-count"><?php echo e($isEdit ? strlen($submission->summary ?? '') : 0); ?></span>/500
                    </div>
                </div>
            </div>
        </div>
            ...
            <div style="display: grid; gap: 20px;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div>
                        ...
                        <input name="company_name" required maxlength="200" 
                               value="<?php echo e($isEdit ? old('company_name', $jobPayload['company_name'] ?? '') : old('company_name')); ?>"
                               placeholder="请输入公司名称" 
                               style="width: 100%; padding: 14px 16px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.15); border-radius: 12px; color: var(--white); font-size: 15px; transition: all 0.2s;"
                               onfocus="this.style.borderColor='var(--primary)'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.15)'"
                               onblur="this.style.borderColor='rgba(255,255,255,0.15)'; this.style.boxShadow='none'">
                    ...
                    <div>
                        ...
                        <input name="job_title" required maxlength="200" 
                               value="<?php echo e($isEdit ? old('job_title', $jobPayload['job_title'] ?? '') : old('job_title')); ?>"
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div>
                        ...
                        <input name="salary_range" maxlength="100" 
                               value="<?php echo e($isEdit ? old('salary_range', $jobPayload['salary_range'] ?? '') : old('salary_range')); ?>"
                    ...
                    <div>
                        ...
                        <input name="location" maxlength="100" 
                               value="<?php echo e($isEdit ? old('location', $jobPayload['location'] ?? '') : old('location')); ?>"
                ...
                <div>
                    ...
                    <textarea name="job_requirements" rows="4" 
                              ...
                              style="width: 100%; padding: 14px 16px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.15); border-radius: 12px; color: var(--white); font-size: 15px; resize: vertical; transition: all 0.2s;"
                              onfocus="this.style.borderColor='var(--primary)'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.15)'"
                              onblur="this.style.borderColor='rgba(255,255,255,0.15)'; this.style.boxShadow='none'"><?php echo e($isEdit ? old('job_requirements', $jobPayload['job_requirements'] ?? '') : old('job_requirements')); ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-section" style="background: var(--dark-light); border-radius: 16px; padding: 24px; margin-bottom: 24px; border: 1px solid rgba(255,255,255,0.08);">
            ...
            <input id="content" type="hidden" name="content" value="<?php echo e($isEdit ? old('content', $submission->content) : old('content')); ?>" required>
        ...
        <div class="form-section" style="background: linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(139, 92, 246, 0.15) 100%); border-radius: 16px; padding: 24px; margin-bottom: 24px; border: 1px solid rgba(99, 102, 241, 0.3);">
            <div style="display: flex; align-items: flex-start; gap: 16px;">
                <label style="display: flex; align-items: flex-start; gap: 14px; cursor: pointer; flex: 1;">
                    <input type="checkbox" name="is_paid" value="1" <?php echo e($isEdit && $submission->is_paid ? 'checked' : ''); ?>
                    ...
                    <div style="flex: 1;">
                        ...
                        <p style="font-size: 13px; color: var(--gray-light); line-height: 1.6; margin: 0;">
                            开启后仅 VIP 用户可观看完整内容，普通用户只能看到摘要。<?php echo e($isEdit ? '修改后可重新提交审核。' : '这是变现的好方式！'); ?>
...
<script>
// 页面加载时初始化
<?php $currentType = $isEdit ? $submission->type : 'document'; ?>
...
<?php if($isEdit): ?>
    document.addEventListener('DOMContentLoaded', function() {
        showTypeFields('<?php echo e($submission->type); ?>');
    });
<?php endif; ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/submissions/create.blade.php ENDPATH**/ ?>